...
clc
clear all
Nx=224;   %pixels along x   CCD像素大小为um,统一单位为um
Ny=224;   %pixels along y
mag=5;
pixel_size=2*mag;%microns  像素大小(um)
...
while data_count < data_cale
    ...
    particle_number=randi(50);%particle number(200以内随机整数)
    sidex=Nx*pixel_size;  %CCD 尺寸
    sidey=Nx*pixel_size;
    pattern=zeros(Nx,Ny);
    particle_size= randi([1,5],1,particle_number); %particle radius  粒子半径list[1,num]
...
Generate hologram   生成全息图 
    ...
    figure(8)
    imshow(I_new);
    filename=['D:\image\train_224\data\',num2str(data_count),'.png'];
...
Generate ground true
    ...
    filename_x=['D:\image\train_224\ture_text/x_',num2str(data_count),'.txt'];
    filename_y=['D:\image\train_224\ture_text/y_',num2str(data_count),'.txt'];
    filename_r=['D:\image\train_224\ture_text/r_',num2str(data_count),'.txt'];
    filename_d=['D:\image\train_224\ture_text/z_',num2str(data_count),'.txt'];
                    ...
                    if  y == y_out(particle_num) && x == x_out(particle_num)   %判断当前点是否为所需要的点
                        ...
                        for x_2_new = x_new:1:x_new+r_out(particle_num)   %对点进行像素级更改
                            for y_2_new = y_new:1:y_new+r_out(particle_num)
                                ...
                                NEW_rgb(x_2_new,y_2_new) = (depth(particle_num)-1000)*z_change;%写入参数
                            end
                        end
    ...
    figure(2)
    imshow(I_new_1);
    filename_1=['D:\image\train_224\label\',num2str(data_count),'.png'];
